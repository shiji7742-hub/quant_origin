...
print("="*60)
print("明日买点快速扫描 (efinance)")
print(f"扫描时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
print("="*60)

def get_stock_data_ef(symbol):
    """使用efinance获取股票历史数据"""
    try:
        df = ef.stock.get_quote_history(symbol, beg='20241101')
...
print(f"\n检查 {len(test_stocks)} 只潜力股的最新信号 (efinance接口)...")
print("-"*60)

results = []
for code, name in test_stocks:
    try:
        df = get_stock_data_ef(code)
