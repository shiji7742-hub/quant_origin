...
try:
    ...
    for i, row in df.head(5).iterrows():
        log(f"    {row['板块名称']} ({row['板块代码']}) 涨跌幅:{row['涨跌幅']}%")
except Exception as e:
    log(f"  获取失败: {e}")
    import traceback
    traceback.print_exc()
...
def detect_potential_sector(daily_returns):
    ...
    if len(daily_returns) < 45:
        return False, None, f"有效数据不足({len(daily_returns)}天)"
    ...
    if recent_15d_sum > 5:
        return False, None, f"近15日涨太多({recent_15d_sum:.1f}%>5%)"
    if recent_5d_sum < -5:
        return False, None, f"近5日跌太多({recent_5d_sum:.1f}%<-5%)"
...
for i, row in df.head(20).iterrows():
    ...
    log(f"      数据: {len(data)}天, 范围{data.index[0].strftime('%Y-%m-%d')}~{data.index[-1].strftime('%Y-%m-%d')}")
...
log("\n" + "="*60)
log(f"诊断完成: 在前20个板块中发现 {len(results)} 个符合条件")
if results:
    log(f"符合条件: {results}")
log("="*60)
