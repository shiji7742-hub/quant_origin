...
# 增加页面数量以覆盖更多股票
max_pages = 50  # 增加到50页，每页80只，约4000只股票
...
stocks = unique_stocks  # 移除数量限制，扫描所有股票
log(f"  去重后共 {len(stocks)} 只股票")

# 扫描放量股票
log(f"\n[2] 全市场扫描放量股票（量比≥{vol_ratio}倍）...")
log(f"  预计扫描 {len(stocks)} 只股票，请耐心等待...")
results = []
...
if HAS_PANDAS and len(results) > 0:
    try:
        ...
        log(f"Excel导出失败: {str(e)} (可能需要安装: pip install pandas openpyxl)")

# 保存为.sel格式（通达信自选股格式）
if len(results) > 0:
    try:
        sel_file = os.path.join(script_dir, f'放量股票_{datetime.now().strftime("%Y%m%d")}.sel')
